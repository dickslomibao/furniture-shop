...
class SystemSettings extends DBConnection
{
	...
	function update_settings_info()
	{
		...
		if (!empty($_FILES['gcash']['tmp_name'])) {

			$ext = pathinfo($_FILES['gcash']['name'], PATHINFO_EXTENSION);
			$fname = "uploads/gcash." . $ext;
			if (move_uploaded_file($_FILES['gcash']['tmp_name'], base_app . $fname)) {
				$qry = $this->conn->query("UPDATE system_info set meta_value = CONCAT('{$fname}', '?v=',unix_timestamp(CURRENT_TIMESTAMP)) where meta_field = 'gcash' ");
			}
		}
